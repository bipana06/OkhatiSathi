...
#                Returns loaded model, optimizer, learning rate, and iteration.
#                learning_rate might be None if not found in the checkpoint.
#     """
#     assert os.path.isfile(checkpoint_path), f"Checkpoint file not found: {checkpoint_path}"
#     print("Loading checkpoint '{}'".format(checkpoint_path))

#     # Load the checkpoint dictionary onto the CPU first
#     checkpoint_dict = torch.load(checkpoint_path, map_location='cpu')

#     # --- Fix for 'module.' prefix issue ---
#     # Check if the state_dict keys start with 'module.' (from DataParallel/DistributedDataParallel)
#     saved_state_dict = checkpoint_dict['state_dict']
#     if any(key.startswith('module.') for key in saved_state_dict.keys()):
#         print("Detected 'module.' prefix in checkpoint keys. Removing prefix...")
...
#                 # (This handles cases where a model might be partially saved/loaded)
#                 new_state_dict[k] = v
#         # Use the corrected state dictionary
#         model_state_dict_to_load = new_state_dict
#     else:
#         # No 'module.' prefix detected, use the state dictionary directly
#         model_state_dict_to_load = saved_state_dict
#     # --- End Fix ---

#     # Load the potentially corrected state dict into the model
#     # Set strict=False to allow loading even if some layers are intentionally ignored (like embedding for warm_start)
#     model.load_state_dict(model_state_dict_to_load, strict=False)
#     print("Model state loaded.")

#     # --- Load Optimizer, Learning Rate, Iteration (Conditional) ---
#     # Initialize return values in case they are missing or warm_start=True is handled later
#     learning_rate = None
#     iteration = 0

#     # Load optimizer state IF the optimizer object is provided AND state exists in checkpoint
#     # Note: The main 'train' function should ideally handle resetting the optimizer
#     #       if warm_start is True, even if we load it here.
#     if optimizer is not None and 'optimizer' in checkpoint_dict:
#         try:
#             optimizer.load_state_dict(checkpoint_dict['optimizer'])
#             print("Optimizer state loaded.")
#         except ValueError as e:
#             print(f"Warning: Could not load optimizer state dict. It might be incompatible.")
#             print(f"         Starting optimizer from scratch. Error: {e}")
#     elif optimizer is not None:
#         print("Warning: Optimizer state not found in checkpoint. Starting optimizer from scratch.")

#     # Load learning rate if it exists
...
#     # Return values consistent with the original function signature in your provided code
#     return model, optimizer, learning_rate, iteration
...
#           rank, group_name, hparams, use_wandb):
...
#                     'epoch': epoch,
#                     'loss': loss.item(),
#                     'lr': learning_rate,  # If you're tracking learning rate
#                 })
